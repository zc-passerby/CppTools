...
* 成功返回行数，失败返回-1
*/
    int RecordSet::ExecuteSQL(string SQL) {
//    cout << "about to query result, sql:" << SQL << endl;
        int nRt;
        unsigned long costtime;
        unsigned long starttime = util::get_current_time_stamp();
        nRt = mysql_real_query(m_Data, SQL.c_str(), strlen(SQL.c_str()));
        if (!nRt) {
            //保存查询结果
            ...
            if (res != NULL && (int) mysql_num_rows(res) > 0) {
                ...
                for (int x = 0; fd = mysql_fetch_field(res); x++) {
                    ...
                    m_field.m_table.push_back(fd->table);
                }
                //保存所有数据
                ...
                if (costtime >= 1000)
                    cout << "Warn:Use " << costtime << "ms to do sql..." << SQL << endl;
                return m_s.size();
            } else {
                m_recordcount = 0;
                m_field_num = 0;
...
/* 获取当前游标的对应字段数据 */
    std::string RecordSet::GetCurrentFieldValue(std::string sFieldName) {
        return m_s[pos][sFieldName].c_str();
    }

    std::string RecordSet::GetCurrentFieldValue(int iFieldNum) {
        if (this)
            return m_s[pos][iFieldNum].c_str();
        else
            return "0";
    }
...
* 返回值: 0成功 -1 失败
*/
    int DataBase::Connect(string host, string user, string passwd, string db, unsigned int port,
                          unsigned int connectTimeout,
                          unsigned int readTimeout, unsigned int writeTimeout, unsigned long client_flag) {
        if (!(m_Data = mysql_init(NULL))) {
            return -3;
        }
        my_bool reconnect = 1;
        mysql_options(m_Data, MYSQL_OPT_RECONNECT, &reconnect);
        mysql_options(m_Data, MYSQL_OPT_READ_TIMEOUT, &readTimeout);
        mysql_options(m_Data, MYSQL_OPT_WRITE_TIMEOUT, &writeTimeout);

        if (mysql_real_connect(m_Data, host.c_str(), user.c_str(), passwd.c_str(), db.c_str(), port, NULL,
                               client_flag)) {
            //选择制定的数据库失败
            ...
            if (!mysql_set_character_set(m_Data, "utf8")) {
//                printf("New client character set: %s\n", mysql_character_set_name(m_Data));
            } else {
                //初始化mysql结构失败
                mysql_close(m_Data);
                m_Data = NULL;
...
//    cout << "about to execute sql with no result,sql:" << sql << endl;

        unsigned long costtime;
        unsigned long starttime = util::get_current_time_stamp();
        int rt = mysql_real_query(m_Data, sql.c_str(), (unsigned long) sql.length());
        if (!rt) {
            //得到受影响的行数
            costtime = (util::get_current_time_stamp() - starttime) / 1000;
            if (costtime >= 1000)
                cout << "Warn:Use " << costtime << "ms to do this sql:" << sql << endl;
            return (int) mysql_affected_rows(m_Data);
        } else {
            //执行查询失败
            const char *err = mysql_error(m_Data); // Returns an empty string
            if (err) {
                cout << "mysql_real_query is failed!!sql:" << sql << ",rt:" << rt << ",err:" << err << endl;
            } else {
                cout << "mysql_real_query is error !!sql:" << sql << ",rt:" << rt << endl;
            }
            return -1;
        }
...
//        costtime = (util::get_current_time_stamp() - starttime) / 1000;
//        if (costtime >= 1000)
//            SqlWarn << "Warn:Use " << costtime << "ms to do this sql:" << sql;
    }
...
*/
    int DataBase::Commit() {
        cout << "enter DataBase::Commit " << endl;
...
*/
    char *DataBase::ExecQueryGetSingValue(string sql) {
        ...
        MYSQL_ROW row;
        char *p = NULL;
//        cout << "about to query single value,sql:" << sql << endl;
...
*/
    int DataBase::Create_db(string name) {
        ...
        temp += name;
        cout << "about to create database :" << name << endl;
...
*/
    int DataBase::Drop_db(string name) {
        ...
        temp = "DROP DATABASE ";
        temp += name;
        cout << "about to drop database :" << name << endl;
        if (!mysql_real_query(m_Data, temp.c_str(),
                              (unsigned long) temp.length()))
            return 0;
        else
